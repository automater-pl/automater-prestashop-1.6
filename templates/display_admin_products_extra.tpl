<div id="automater" class="panel product-tab">
    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            <select name="selautomater_product_id"  id="selautomater_product_id"  style="width:100%;">
                <option value="0">{l s="--- select value ---" mod='automater'}</option>
                {foreach from=$automaterProducts item=product}
                    <option value="{$product.id}" {if $assignedProductId == $product.id}selected="selected"{/if}>ID {$product.id}: {$product.name}</option>
                {/foreach}
            </select>
        </div>
    </div>
